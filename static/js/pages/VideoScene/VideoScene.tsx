...
const COS_PREFIX = 'https://smartcontent-source-1257411467.cos.accelerate.myqcloud.com/assets/videoTemporal'
...
async function uploadVideo(file: File) {
  const cosKey = `${BACK_ENV}/video/videoScene/${file.size}-${file.name}`
...
const VideoScene: React.FC = () => {
  ...
  const { t } = useTranslation('videoScene')
      ...
      try {
        ...
        if (job && job.state === 3) {
          const resultJsonUrl = await getTempUrl(
            `${BACK_ENV}/result/videoScene/${job.outputs[0].smartContentResult?.videoAnalysis?.detectionFileName}`,
          )
  ...
  return (
    <div className="VideoScene">
      ...
      >
        <div className="VideoScene-banner-content">
          <h2 className="VideoScene-banner-title">{t`videoScene`}</h2>
          <p className="VideoScene-banner-description">{t`intro`}</p>
        </div>
      </div>

      <div className="VideoScene-demo">
        <div className="VideoScene-main">
          <div className="VideoScene-preview">
            <h2 className="VideoScene-main-title">{t`inputVideo`}</h2>
            <div className="VideoScene-preview-wrap">
              <video src={previewURL} controls controlsList="nodownload" ref={videoPreviewRef}></video>
            </div>
          </div>
          <div className="VideoScene-result">
            <h2 className="VideoScene-main-title">{t`detectionResult2`}</h2>
            <div
              className={cn('VideoScene-result-wrap', {
                loading: loadingTuuid || (!result && !videoMSG),
              })}
            >
              {videoMSG && !result ? (
                <div className="VideoScene-result-message">{`${t`tip.processedFailed`} (${videoMSG})`}</div>
              ) : 
              (result && result.length>0 ? 
                (<textarea className="VideoScene-result-json" value={formatJson(result)} readOnly></textarea>) :
                (<div className="VideoScene-result-message">{`${t`tip.emptyResult`}`}</div>)
              )}
              <div className="VideoScene-result-cover">
                <Spinner />
                <p className="VideoScene-result-cover-status">
        ...
        <div className="VideoScene-sample">
          <ul className="VideoScene-sample-list">
            {demos.map(({ isSample, url }, index) => (
              <li
                key={url}
                className={cn('VideoScene-sample-item', { active: url === previewURL })}
                onClick={() => onChangeSample(demos[index])}
              >
                <video src={url}></video>
                {isSample ? null : (
                  <span className="VideoScene-sample-item-delete" onClick={(e) => onDelete(index, e)}></span>
                )}
              </li>
            ))}
            <li className={cn('VideoScene-sample-item', 'uploader')}>
