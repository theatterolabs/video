...
import { readFile, POLLING_INTERVAL, BACK_ENV, putObjectSlice, getTempUrl, createJob, getJob, CDN_COS } from 'utils'
...
const COS_PREFIX = 'https://smartcontent-source-1257411467.cos.accelerate.myqcloud.com/assets/videoCrop'
const demos: Demo[] = [
  {
...
async function uploadVideo(file: File) {
  const cosKey = `${BACK_ENV}/video/crop/${file.size}-${file.name}`
  ...
  return await createJob(
    { url },
    {
      outputPrefix: `${file.size}-${Math.random().toString(36).slice(-6)}`,
      videoCrop: {
        width: 100,
        height: 178,
      },
    },
    `${BACK_ENV}/result/videoCrop`,
  )
}
...
const VideoCrop: React.FC = () => {
  ...
  const { t } = useTranslation('videoCrop')
      ...
      try {
        ...
        if (job && job.state === 3) {
          const resultUrl = await getTempUrl(
            `${BACK_ENV}/result/videoCrop/${job.outputs[0].smartContentResult?.videoCrop}`,
          )
          if (resultUrl) {
  ...
  return (
    <div className="VideoCrop">
      ...
      >
        <div className="VideoCrop-banner-content">
          <h2 className="VideoCrop-banner-title">{t`videoCrop`}</h2>
          <p className="VideoCrop-banner-description">{t`intro`}</p>
        </div>
      </div>

      <div className="VideoCrop-demo">
        <div className="VideoCrop-main">
          <div className="VideoCrop-preview">
            <h2 className="VideoCrop-main-title">{t`originalVideo`}</h2>
            <div className="VideoCrop-preview-wrap">
              <video src={previewURL} controls controlsList="nodownload" ref={videoPreviewRef}></video>
            </div>
          </div>
          <div className="VideoCrop-result">
            <h2 className="VideoCrop-main-title">{t`processedVideo`}</h2>
            <div
              className={cn('VideoCrop-result-wrap', {
                loading: loadingTuuid || (!result && !videoMSG),
              })}
            >
              {videoMSG && !result ? (
                <div className="VideoCrop-result-message">{`${t`tip.processedFailed`} (${videoMSG})`}</div>
              ) : (
                <video src={result} controls controlsList="nodownload" ref={videoResultRef}></video>
              )}
              <div className="VideoCrop-result-cover">
                <Spinner />
                <p className="VideoCrop-result-cover-status">
        ...
        <div className="VideoCrop-sample">
          <ul className="VideoCrop-sample-list">
            {demos.map(({ isSample, url }, index) => (
              <li
                key={url}
                className={cn('VideoCrop-sample-item', { active: url === previewURL })}
                onClick={() => onChangeSample(demos[index])}
              >
                <video src={url}></video>
                {isSample ? null : (
                  <span className="VideoCrop-sample-item-delete" onClick={(e) => onDelete(index, e)}></span>
                )}
              </li>
            ))}
            <li className={cn('VideoCrop-sample-item', 'uploader')}>
